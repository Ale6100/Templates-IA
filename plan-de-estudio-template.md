...
**Esto también aplica a versiones de herramientas/librerías/lenguajes presentes en el material**: si el código o los apuntes usan una versión específica, confirmarla (manifiesto de dependencias, encabezados de versión, changelog citado en el material) antes de explicar su comportamiento de memoria — una explicación genérica puede no coincidir con la versión real del material, sobre todo si es más nueva o más vieja que la más conocida en general. Si hace falta, buscar en internet la documentación de esa versión puntual en vez de asumir por conocimiento propio (que tiene una fecha de corte).
