...
- El README debe ser **completo pero conciso**: tiene que cubrir todo lo necesario para entender y desarrollar el proyecto, pero sin redundancias ni explicaciones más detalladas de lo que aportan. Describí qué existe, qué patrón se usa y el *porqué* de las decisiones; lo que se deduce leyendo el código en segundos (listados exhaustivos, pasos triviales, repetir lo que ya dice otra sección) no va. Tampoco traces la cadena de llamadas entre funciones o archivos (qué función llama a cuál, qué archivo le pasa qué a otro, en qué orden): duplica el código y queda vieja con cualquier cambio interno. Si te encontrás describiendo un flujo interno en vez de un concepto, contrato o decisión, resumilo en una o dos oraciones o sacalo.
...
Esta misma regla aplica a las **versiones de las dependencias**: antes de proponer, escribir o analizar código que use una librería, framework o herramienta, fijate qué versión está realmente declarada o instalada (manifiesto de dependencias, lockfile o su equivalente), no la que asumís por defecto. Tu conocimiento puede fallar en dos direcciones: sugerir una API de una versión más nueva que la del proyecto, o ignorar cambios de una versión posterior a tu fecha de corte. Si tenés dudas sobre cómo se comporta esa versión puntual y podés buscar en internet, confirmalo ahí en vez de asumir.
